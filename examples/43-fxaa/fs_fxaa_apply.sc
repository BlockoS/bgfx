// This is a stripped down of fxaa 3.11 (fxaaConsole, 360 and PS3 versions have been removed.) 
$input v_texcoord0

#include <bgfx_shader.sh>

SAMPLER2D(s_albedo, 0);
// {x___} = fxaaQualitySubpix
// {_y__} = fxaaQualityEdgeThreshold
// {__z_} = fxaaQualityEdgeThresholdMin
// {___w} = unused
...
void main() {
    vec2 rcp_frame = rcp(vec2(textureSize(s_albedo, 0)));

    gl_FragColor = FxaaPixelShader(
        v_texcoord0,    // vec2 pos,
        s_albedo,       // sampled2D tex,
        rcp_frame,      // vec2 fxaaQualityRcpFrame,
        u_fxaaParams.x, // float fxaaQualitySubpix,
        u_fxaaParams.y, // float fxaaQualityEdgeThreshold,
        u_fxaaParams.z  // float fxaaQualityEdgeThresholdMin
    );
}